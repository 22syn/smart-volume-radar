...
export async function fetchAllStocksAsOfDate(
    ...
    const tasks = tickers.map((ticker, index) =>
        limit(async () => {
            ...
            logger.warn(`❌ ${ticker}: No data from any source (Yahoo or Twelve Data) as of ${asOfDate}. Check for typos, exchange suffixes (e.g. .L, .SA, .TA, .T, .KS, .DE, .MI, .TO, .V), or format differences (e.g. BRK-B vs BRK.B).`);
            return { ticker, data: null };
        })
    );
...
export async function fetchAllStocks(tickers: string[]): Promise<FetchAllStocksResult> {
    ...
    const tasks = tickers.map((ticker, index) => limit(async () => {
        ...
        if (result) {
            ...
            logger.warn(`❌ ${ticker}: No data from any source (Yahoo or Twelve Data). Check for typos (e.g. COBE vs CBOE), exchange suffixes (e.g. .L, .SA, .TA, .T, .KS, .DE, .MI, .TO, .V), or format differences (e.g. BRK-B vs BRK.B).`);
            return { ticker, data: null };
        }
    }));
