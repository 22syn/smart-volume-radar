...
/** Valid ticker: optional ^, 1-16 alphanumeric chars/dashes, optional .XXXXX exchange suffix */
const TICKER_REGEX = /^\^?[A-Za-z0-9-]{1,16}(\.[A-Za-z0-9]{1,5})?$/;
...
export function validateTicker(ticker: string): boolean {
    const t = ticker.trim();
    return t.length > 0 && t.length <= 25 && TICKER_REGEX.test(t);
}
