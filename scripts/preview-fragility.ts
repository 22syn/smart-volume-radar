...
async function main(): Promise<void> {
    ...
    logger.info('date        score  core3 climax  wick10 %>50  dist20 ext50  corr20 disp10  index   DD%   canary  nearHigh');
    for (const d of result.series.slice(-10)) {
        logger.info(
            `${d.date}  ${fmt(d.score)}  ${fmt(d.core3)}  ${fmt(d.climax)}  ${fmt(d.z.wick10)} ${fmt(d.z.pctAbove50)} ${fmt(d.z.dist20)} ` +
    ...
    logger.info(`🟣 Purple Fragility: ${emoji} ${s.toFixed(2)} (סף ${FRAGILITY_THRESHOLD.toFixed(1)})${core3Bit}${climaxBit} | DD ${result.latest.drawdownPct.toFixed(1)}%${canaryBit}`);
