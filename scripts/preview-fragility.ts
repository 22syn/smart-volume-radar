 ...
 */
import { computePurpleFragility, FRAGILITY_THRESHOLD, CORE3_THRESHOLD, CORE3_WATCH_DISPLAY } from '../src/services/purpleFragility.js';
...
async function main(): Promise<void> {
    ...
    logger.info('date        score  core3  wick10 %>50  dist20 ext50  corr20 disp10  index   DD%   canary');
    for (const d of result.series.slice(-10)) {
        logger.info(
            `${d.date}  ${fmt(d.score)}  ${fmt(d.core3)}  ${fmt(d.z.wick10)} ${fmt(d.z.pctAbove50)} ${fmt(d.z.dist20)} ` +
    ...
    logger.info(
        ...
        `core3=${result.latest.core3?.toFixed(2) ?? '—'} | crossedUp=${result.crossedUp} | ` +
        `core3CrossedUp=${result.core3CrossedUp} | threshold=${FRAGILITY_THRESHOLD}`
    );
    ...
    logger.info(`🟣 Purple Fragility: ${emoji} ${s.toFixed(2)} (סף ${FRAGILITY_THRESHOLD.toFixed(1)})${core3Bit} | DD ${result.latest.drawdownPct.toFixed(1)}%${canaryBit}`);
